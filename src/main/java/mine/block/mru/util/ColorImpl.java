package mine.block.mru.util;

public class ColorImpl {
    private final int color;

    private ColorImpl(int color) {
        this.color = color;
    }

    public static ColorImpl ofTransparent(int color) {
        return new ColorImpl(color);
    }

    public static ColorImpl ofOpaque(int color) {
        return new ColorImpl(-16777216 | color);
    }

    public static ColorImpl ofRGB(float r, float g, float b) {
        return ofRGBA(r, g, b, 1.0F);
    }

    public static ColorImpl ofRGB(int r, int g, int b) {
        return ofRGBA(r, g, b, 255);
    }

    public static ColorImpl ofRGBA(float r, float g, float b, float a) {
        return ofRGBA((int)((double)(r * 255.0F) + 0.5D), (int)((double)(g * 255.0F) + 0.5D), (int)((double)(b * 255.0F) + 0.5D), (int)((double)(a * 255.0F) + 0.5D));
    }

    public static ColorImpl ofRGBA(int r, int g, int b, int a) {
        return new ColorImpl((a & 255) << 24 | (r & 255) << 16 | (g & 255) << 8 | b & 255);
    }

    public static ColorImpl ofHSB(float hue, float saturation, float brightness) {
    ...
    public ColorImpl brighter(double factor) {
        int r = this.getRed();
        int g = this.getGreen();
        int b = this.getBlue();
    ...
    public ColorImpl darker(double factor) {
    ...
    public boolean equals(Object other) {
        if (this == other) {
            ...
            return this.color == ((ColorImpl)other).color;
        } else {
            return false;
        }
    }
    ...
    public String toString() {
        return "ColorImpl{r=" + this.getRed() + "g=" + this.getGreen() + "b=" + this.getBlue() + '}';
    }
}
